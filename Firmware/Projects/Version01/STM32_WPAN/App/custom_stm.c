...
typedef struct{
  ...
  uint16_t  CustomOtaservHdle;                    /**< OTAService handle */
  uint16_t  CustomRebootHdle;                  /**< RebootCharacteristic handle */
}CustomContext_t;
...
static const uint8_t SizeReboot=3;
/**
 ...
 */
#define COPY_OTASERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x20,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_REBOOTCHARACTERISTIC_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x11,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
static SVCCTL_EvtAckStatus_t Custom_STM_Event_Handler(void *Event)
{
  ...
  APP_DBG_MSG("Custom_STM_Event_Handler\n");
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch(blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if(attribute_modified->Attr_Handle == (CustomContext.CustomRebootHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE :
          ...
          else if(read_req->Attribute_Handle == (CustomContext.CustomBatlvlHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            Notification.Custom_Evt_Opcode = CUSTOM_STM_BATLVL_READ_EVT;
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
          ...
          if(write_perm_req->Attribute_Handle == (CustomContext.CustomRebootHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            Notification.Custom_Evt_Opcode = CUSTOM_STM_REBOOT_WRITE_EVT;
            Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *
   * Max_Attribute_Records = 1 + 2*1 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for OTAService +
   *                                2 for RebootCharacteristic +
   *                              = 3
   */

  COPY_OTASERVICE_UUID(uuid.Char_UUID_128);
  aci_gatt_add_service(UUID_TYPE_128,
                       (Service_UUID_t *) &uuid,
                       PRIMARY_SERVICE,
                       3,
                       &(CustomContext.CustomOtaservHdle));

  /**
   *  RebootCharacteristic
   */
  COPY_REBOOTCHARACTERISTIC_UUID(uuid.Char_UUID_128);
  aci_gatt_add_char(CustomContext.CustomOtaservHdle,
                    UUID_TYPE_128, &uuid,
                    SizeReboot,
                    CHAR_PROP_WRITE,
                    ATTR_PERMISSION_NONE,
                    ...
                    0x10,
                    CHAR_VALUE_LEN_CONSTANT,
                    &(CustomContext.CustomRebootHdle));
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch(CharOpcode)
  {
    ...
    case CUSTOM_STM_REBOOT:
      result = aci_gatt_update_char_value(CustomContext.CustomOtaservHdle,
                                          CustomContext.CustomRebootHdle,
                                          0, /* charValOffset */
                                          SizeReboot, /* charValueLen */
                                          (uint8_t *)  pPayload);
